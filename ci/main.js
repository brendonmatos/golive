const GO_LIVE_CONNECTED = "gl-connected";
const GO_LIVE_COMPONENT_ID = "gl-cid";
...
const goLive = {
    ...
    connectElement(viewElement) {
        ...
        keydownElements.forEach(function (element) {
            ...
            const componentId = getComponentIdFromElement(element);
            const method = element.getAttribute("gl-keydown");
            ...
            for (let i = 0; i < attrs.length; i++) {
                if (
                    attrs[i].name === "gl-key" ||
                    attrs[i].name.startsWith("gl-key-")
                ) {
...
const findLiveInputsFromElement = (el) => {
    return el.querySelectorAll(
        ["*[gl-input]:not([", GO_LIVE_CONNECTED, "])"].join("")
    );
};

const findLiveClicksFromElement = (el) => {
    return el.querySelectorAll(
        ["*[gl-click]:not([", GO_LIVE_CONNECTED, "])"].join("")
    );
};

const findLiveKeyDownFromElement = (el) => {
    return el.querySelectorAll(
        ["*[gl-keydown]:not([", GO_LIVE_CONNECTED, "])"].join("")
    );
};
...
const dataFromElementAttributes = (el) => {
    ...
    let data = {};
    for (let i = 0; i < attrs.length; i++) {
        if (attrs[i].name.startsWith("gl-data-")) {
            data[attrs[i].name.substring(8)] = attrs[i].value;
        }
    }
...
const getComponentIdFromElement = (element) => {
    const attr = element.getAttribute("gl-cid");
    if (attr) {
        return attr;
    }
